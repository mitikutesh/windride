...
import { parseTraceToFixes, ReplaySource } from '../src/nav/replay';
...
if (!file) {
  console.error('usage: npm run replay -- <file.gpx> [--speed N] [--jitter metres]');
  process.exit(1);
}
...
const source = new ReplaySource(fixes, { speed, jitterM });
let n = 0;
source.start((fix) => {
  n++;
