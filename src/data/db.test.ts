...
describe('routes store (idb)', () => {
  ...
  it('v5 schema smoke: a second connection sees every store (incl. config) and the record', async () => {
    await saveRoute(route('smoke', 3000));
    await openWindrideDb(); // first open (via the app helper) creates v5
    // A genuinely separate second connection at v5 sees every store and the persisted record.
    const again = await openDB('windride', 5, {
      upgrade(db) {
    ...
    expect(again.objectStoreNames.contains('config')).toBe(true);
